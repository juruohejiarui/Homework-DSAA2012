...
def get_valid(preds : torch.Tensor, targets : torch.Tensor, mask : torch.Tensor) -> tuple[torch.Tensor, torch.Tensor] :
	...
	batch_size, seq_len, tone_vocab_size = preds.shape
	mask_flat = mask.view(-1)               # (batch_size * seq_len)
	preds_flat = preds.view(-1, tone_vocab_size)   # (batch_size * seq_len, tone_vocab_size)
	targets_flat = targets.view(-1)         # (batch_size * seq_len)

	preds_flat = preds_flat[mask_flat == 1]     # (num_valid, tone_vocab_size)
...
def get_metrics(preds : torch.Tensor, targets : torch.Tensor) -> dict :
	# preds : (num_valid, tone_vocab_size)
	# targets : (num_valid,)
...
def train_epoch(model : nn.Module, dataLoader : torch.utils.data.DataLoader, criterion, optimizer, scheduler = None) -> dict :
	...
	for prev_durr, prev_pitc, prev_tone, curr_durr, curr_pitc, curr_tone, prev_mask, curr_mask in dataLoader :
		...
		# model = model.cpu()
		optimizer.zero_grad()
		logits : torch.Tensor = model(prev_durr, prev_pitc, prev_tone, curr_durr, curr_pitc, prev_mask, curr_mask)   # (batch_size, seq_len, tone_vocab_size)
		preds_flat, targets_flat = get_valid(logits, curr_tone, curr_mask)
		loss : torch.Tensor = criterion(preds_flat, targets_flat)
		loss.backward()
		optimizer.step()
...
def split_data(dataset : data.Dataset, train_size : int) -> torch.utils.data.Dataset :
...
if __name__ == "__main__" :
	...
	if split_item :
		dataset = data.Dataset(data_dir="dataset/Humdrum-files", split=1, train=True)
		...
		train_dataset = data.Dataset(data_dir="dataset/Humdrum-files", split=split_rate, train=True)
		val_dataset = data.Dataset(data_dir="dataset/Humdrum-files", split=split_rate, train=False)
	...
	logger = SummaryWriter(logdir="logs/eval_model", flush_secs=10)
	...
	for epoch in tqdm(range(epoch_num)) :
		...
		if epoch >= epoch_num / 10 and (bst_metric is None or val_metrics['f1'] > bst_metric['f1']) :
			...
			torch.save(bst_model, "pretrained_models/best_eval_model.pth")			
			need_log = True
	...
	torch.save(bst_model, "pretrained_models/final_eval_model.pth")